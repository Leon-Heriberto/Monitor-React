...
import { faAngleDown, faAngleUp, faCalendar, faChartArea, faChartBar, faChartLine, faFlagUsa, faFolderOpen, faGlobeEurope, faPaperclip, faUserPlus } from '@fortawesome/free-solid-svg-icons';
...
export const CounterWidget = (props) => {
  const { icon, iconColor, category, title, period, percentage, range } = props;
  const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
  const percentageColor = percentage < 0 ? "text-danger" : "text-success";

  const iconName = icon === 'faThumbsUp' ? faThumbsUp : 
                   icon === 'faThumbsDown' ? faThumbsDown : faChartBar
      ...
      <Card.Body>
        <Row className="d-block d-xl-flex align-items-center">
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            ...
            <small><FontAwesomeIcon icon={faCalendar} size="xs" /> {period} </small>
  ...
  return (
    <Card border="light" className="shadow-sm">
      ...
      <Card.Body className="p-2">
        <BarChart labels={labels[0]} series={series} />
      </Card.Body>
    </Card>
  );
};
      ...
      <Card.Header className="d-flex flex-row align-items-center flex-0">
        <div className="d-block">
          ...
          <small className="fw-bold mt-2">
            <span className="me-2">Últimos 20 minutos</span>
          </small>
        </div>
        <div className="d-flex ms-auto">
          <Button variant="primary" size="sm" className="me-2">Actualizar</Button>
        </div>
      </Card.Header>
    ...
    <Card className="shadow-sm">
      <Card.Header className="d-md-flex flex-row align-items-center flex-0">
        <div className="d-block mb-3 mb-md-0">
          ...
          <h3>{value}</h3>
          <small className="fw-bold mt-2">
            <span className="me-2">Últimas 3 horas</span>
            <FontAwesomeIcon icon={percentageIcon} className={`${percentageColor} me-1`} />
